    ...
    <main className="min-h-screen relative overflow-x-hidden flex flex-col font-mono selection:bg-indigo-500/30 text-slate-900 dark:text-slate-100 bg-[#F5F7FF] dark:bg-[#050505] transition-colors duration-300">
      ...
      <section className="relative w-full h-screen flex flex-col items-center justify-center z-10 overflow-hidden">
          ...
          <svg viewBox="0 0 1000 700" className="w-[150vw] h-[150vh] min-w-[1200px] min-h-[900px] max-w-none">
            ...
            <g transform="translate(500, 150)">
              ...
              <g className="body-group">
                ...
                {/* 3. LEGS */}
                <polygon points="-60,350 60,350 40,550 -40,550" fill="rgba(79,70,229,0.03)" stroke="rgba(79,70,229,0.6)" strokeWidth="2" strokeLinejoin="round" />
                <polygon points="-40,550 40,550 20,700 -20,700" fill="rgba(79,70,229,0.03)" stroke="rgba(79,70,229,0.5)" strokeWidth="2" strokeLinejoin="round" />
        ...
        <div className="text-center flex flex-col items-center gap-4 mt-16 px-4 z-10 backdrop-blur-sm p-8 rounded-3xl bg-white/30 dark:bg-black/30 border border-white/40 dark:border-white/10 shadow-xl shadow-indigo-500/5 transition-colors duration-300">
          ...
          <h1 className="text-[12vw] sm:text-[10vw] md:text-[9vw] lg:text-[140px] font-black uppercase tracking-tighter leading-none flex items-center justify-center relative mt-4">
            {typedTitle.split('').map((char, index) => {
              const isAI = index === 1 || index === 3; // "A" and "I"
              return (
                <span 
                  key={index} 
                  className={isAI 
                    ? "text-indigo-600 dark:text-indigo-400 drop-shadow-[0_0_15px_rgba(99,102,241,0.5)] mx-0.5" 
                    : "text-slate-900 dark:text-white inline-block"
                  }
                >
                  {char}
                </span>
        ...
        <button 
          ...
          className="absolute bottom-12 flex flex-col items-center gap-2 text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors animate-bounce cursor-pointer group z-20"
        >
      ...
      <div id="platform" className="w-full max-w-7xl mx-auto p-4 sm:p-8 z-10 space-y-32 pb-32 relative">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pt-16 reveal-on-scroll opacity-0 translate-y-12 transition-all duration-1000 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
              ...
              {success ? (
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                  <label className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-2">
                    <TerminalSquare size={14}/> Inserisci la tua Email
                  </label>
                  <div className="relative flex items-center">
                    ...
                    <input 
                      ...
                      placeholder="coach@calisthenics.it"
                      className="w-full bg-slate-50 dark:bg-black/20 border border-indigo-100 dark:border-white/10 rounded-xl py-4 pl-10 pr-4 text-base text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600"
                    />
                  </div>
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 dark:disabled:bg-white/10 text-white text-sm font-black uppercase tracking-[0.2em] rounded-xl transition-all shadow-md hover:shadow-xl hover:shadow-indigo-500/20 flex items-center justify-center gap-3 relative overflow-hidden group"
                  >
          ...
          <div className="lg:col-span-6 relative">
            ...
            <div className="bg-white/90 dark:bg-black/80 border border-indigo-100 dark:border-white/10 rounded-3xl p-6 relative shadow-2xl shadow-indigo-500/10 dark:shadow-none overflow-hidden flex flex-col h-[500px] backdrop-blur-xl transition-colors duration-300">
              ...
              <div className="flex-1 overflow-hidden flex flex-col gap-5">
                ...
                <div className="space-y-4 flex-1 overflow-auto pr-2 custom-scrollbar">
                  {[
                    { title: "High Pull-ups Esplosivi", desc: "Trazione al petto, focus potenza", sets: "5x", reps: "3-4" },
                    { title: "Pull-ups Zavorrati", desc: "RPE 8 - Pausa 120s per SNC", sets: "4x", reps: "5-6" },
                    { title: "Dip agli Anelli", desc: "Controllo eccentrico estremo", sets: "3x", reps: "8-10" },
                    { title: "Hollow Body Holds", desc: "Massimo tensionamento del core", sets: "3x", reps: "15s" }
                  ].map((ex, i) => (
                    <div key={i} className="p-4 bg-slate-50/50 dark:bg-white/5 border border-indigo-50 dark:border-white/5 rounded-xl flex items-center justify-between hover:border-indigo-300 dark:hover:border-indigo-500/30 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="text-sm font-bold text-slate-900 dark:text-slate-200 uppercase">{ex.title}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-500 mt-0.5">{ex.desc}</div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-black text-indigo-600 dark:text-indigo-400">{ex.sets}</div>
                        <div className="text-xs font-bold text-slate-400 dark:text-slate-600 uppercase">{ex.reps}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-5 pt-5 border-t border-indigo-50 dark:border-white/5 flex justify-between items-center">
                <div className="text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                  <CheckSquare size={14} className="text-emerald-500" /> Dati Biomeccanici Applicati
                </div>
                <button className="px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-black text-xs font-black uppercase rounded-lg hover:opacity-80 transition-colors">
                  Invia all'Allievo
                </button>
              </div>
            </div>
        ...
        {/* === PAIN POINT SECTION === */}
        <section className="bg-white/50 dark:bg-white/5 border border-indigo-100 dark:border-white/10 rounded-3xl p-8 lg:p-16 shadow-sm reveal-on-scroll opacity-0 translate-y-12 transition-all duration-1000 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-500 dark:text-slate-400 mt-4 max-w-2xl mx-auto">I Personal Trainer passano più tempo a sistemare caselle su Excel che a correggere la tecnica dei propri allievi. È ora di evolversi.</p>
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            <div className="p-8 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-500/20 rounded-2xl space-y-6 hover:shadow-lg transition-all duration-500">
              <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400 mb-6 border-b border-rose-100 dark:border-rose-500/20 pb-4">
                <XCircle size={24} />
                <h3 className="text-xl font-bold text-rose-900 dark:text-rose-300 uppercase tracking-widest">Oggi (Il Problema)</h3>
              </div>
              <ul className="space-y-4 text-sm text-rose-900/80 dark:text-rose-200/70">
                <li className="flex gap-3"><span className="text-rose-500 font-bold">×</span> Perdi 40 minuti per creare una singola scheda da zero.</li>
                <li className="flex gap-3"><span className="text-rose-500 font-bold">×</span> Calcoli i volumi a mente, rischiando di far sovrallenare l'allievo.</li>
                <li className="flex gap-3"><span className="text-rose-500 font-bold">×</span> Formattazione su Excel noiosa e poco professionale da vedere su smartphone.</li>
                <li className="flex gap-3"><span className="text-rose-500 font-bold">×</span> Impossibile gestire più di 15-20 clienti senza impazzire.</li>
              </ul>
            </div>

            <div className="p-8 bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl space-y-6 relative overflow-hidden hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-500">
              <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-200/20 blur-2xl"></div>
              <div className="flex items-center gap-3 text-indigo-600 dark:text-indigo-400 mb-6 border-b border-indigo-100 dark:border-indigo-500/20 pb-4">
                <CheckSquare size={24} />
                <h3 className="text-xl font-bold text-indigo-900 dark:text-white uppercase tracking-widest">Domani (CALI.AI)</h3>
              </div>
              <ul className="space-y-4 text-sm text-indigo-900/80 dark:text-indigo-100 font-medium">
                <li className="flex gap-3"><span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span> La IA crea la base perfetta in 30 secondi netti.</li>
                <li className="flex gap-3"><span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span> Algoritmo che bilancia spinta/tirata e previene infortuni articolari.</li>
                <li className="flex gap-3"><span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span> Output visivo perfetto, pronto per essere letto dall'allievo.</li>
                <li className="flex gap-3"><span className="text-indigo-600 dark:text-indigo-400 font-bold">✓</span> Gestisci 50+ atleti decuplicando il tuo margine di guadagno.</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="relative py-12 reveal-on-scroll opacity-0 translate-y-12 transition-all duration-1000 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
          <div className="text-center mb-20 space-y-4">
            <h2 className="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.4em] flex items-center justify-center gap-2">
              <Layers size={18} /> Workflow Ottimizzato
            </h2>
            <h3 className="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Come si lavora su CALI.AI</h3>
          </div>

          <div className="grid md:grid-cols-3 gap-8 relative">
            <div className="hidden md:block absolute top-[40px] left-[20%] right-[20%] h-px bg-indigo-100 dark:bg-white/10 border-t border-dashed border-indigo-200 dark:border-indigo-500/30 z-0"></div>

            {/* FASE 1 */}
            <div className="relative z-10 bg-white dark:bg-white/5 p-8 rounded-3xl border border-indigo-100 dark:border-white/10 shadow-lg hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-all group">
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/30 flex items-center justify-center mb-8 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                <Database size={28} />
              </div>
              <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-wider mb-3 flex items-center gap-2">
                <span className="text-indigo-500 dark:text-indigo-400 text-sm">01.</span> Inserisci i Dati
              </h4>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                Profila l'allievo in pochi click. Indica il suo livello, gli attrezzi che ha a casa (es. "solo sbarra" o "niente") e il suo obiettivo principale.
              </p>
            </div>

            {/* FASE 2 */}
            <div className="relative z-10 bg-white dark:bg-white/5 p-8 rounded-3xl border border-indigo-100 dark:border-white/10 shadow-lg hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-all group" style={{ transitionDelay: '100ms' }}>
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/30 flex items-center justify-center mb-8 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                <BrainCircuit size={28} />
              </div>
              <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-wider mb-3 flex items-center gap-2">
                <span className="text-indigo-500 dark:text-indigo-400 text-sm">02.</span> Calcolo Neurale
              </h4>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                L'IA processa migliaia di paper scientifici (EBT). Calcola istantaneamente la frequenza ottimale, i tempi di recupero e gli esercizi propedeutici.
              </p>
            </div>

            {/* FASE 3 */}
            <div className="relative z-10 bg-white dark:bg-white/5 p-8 rounded-3xl border border-indigo-100 dark:border-white/10 shadow-lg hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-all group" style={{ transitionDelay: '200ms' }}>
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/30 flex items-center justify-center mb-8 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                <Activity size={28} />
              </div>
              <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-wider mb-3 flex items-center gap-2">
                <span className="text-indigo-500 dark:text-indigo-400 text-sm">03.</span> Revisione Artigianale
              </h4>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                Ottieni un programma solido al 90%. Usa il tuo tocco da Coach per modificare gli ultimi dettagli in 5 minuti, e condividi il link con l'allievo.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* === DEEP DIVE / TERMINAL === */}
        <section className="bg-white dark:bg-black/20 border border-indigo-100 dark:border-white/10 rounded-[2.5rem] p-8 lg:p-16 relative overflow-hidden shadow-xl reveal-on-scroll opacity-0 translate-y-12 transition-all duration-1000 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
          <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none text-indigo-900 dark:text-white">
            <Cpu size={300} />
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 relative z-10 items-center">
            <div className="space-y-10">
              <div>
                <h2 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tight mb-6 leading-tight">Il primo algoritmo che "pensa" come un Coach</h2>
                <p className="text-base text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
                  CALI.AI non genera schede a caso. Abbiamo addestrato il modello sui fondamenti dell'Evidence-Based Training specifici per il corpo libero.
                </p>
              </div>

              <div className="space-y-8">
                <div className="flex gap-5 items-start">
                  <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl border border-indigo-100 dark:border-indigo-500/30 shrink-0 text-indigo-600 dark:text-indigo-400">
                    <ShieldAlert size={20} />
                  </div>
                  <div>
                    <h4 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-wide">Tutela Tendini e Articolazioni</h4>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 leading-relaxed font-medium">Le zavorre e le isometrie (Planche, Iron Cross) distruggono il SNC. Il software dosa l'intensità per evitare infortuni tipici del Calisthenics.</p>
                  </div>
                </div>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 font-mono text-[11px] sm:text-xs text-indigo-200 shadow-2xl flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="space-y-4 font-bold tracking-wide">
                <p><span className="text-indigo-400">{">"}</span> <span className="text-white">ANALISI_RICHIESTA</span> --coach="Admin"</p>
                <p className="text-slate-400 pl-4">Allievo: Marco (30 anni)</p>
                <p className="text-slate-400 pl-4">Disponibilità: 3 giorni a settimana</p>
                
                <p className="pt-2"><span className="text-indigo-400">{">"}</span> <span className="text-white">ELABORAZIONE_PARAMETRI...</span></p>
                <p className="text-amber-400 pl-4 animate-pulse">ATTENZIONE: Fatica SNC rilevata nella sessione B.</p>
                <p className="text-amber-400 pl-4">Ricalcolo volume. Spostamento isometrie a inizio sessione.</p>
                
                <p className="pt-2"><span className="text-indigo-400">{">"}</span> <span className="text-white">GENERAZIONE_COMPLETATA</span></p>
                <div className="pl-4 border-l-2 border-emerald-500/50 mt-2 space-y-1">
                   <p className="text-emerald-400">[SUCCESS] Distribuzione Sets: Push (45%) / Pull (55%)</p>
                   <p className="text-emerald-400">[SUCCESS] Rischio infortuni ridotto del 32%</p>
                   <p className="text-emerald-400">[SUCCESS] Scheda pronta per l'esportazione.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* BOTTOM CTA */}
        <section className="text-center py-16 space-y-8 reveal-on-scroll opacity-0 translate-y-12 transition-all duration-1000 ease-out [&.is-visible]:opacity-100 [&.is-visible]:translate-y-0">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-500/30 rounded-full text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-widest mx-auto">
            <Lock size={14} /> Posti Beta Limitati
          </div>
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-xl mx-auto font-medium">Non continuare a sprecare il tuo tempo. Iscriviti alla waitlist per essere tra i primi coach in Italia a utilizzare l'Intelligenza Artificiale nel Calisthenics.</p>
          <a href="#waitlist" className="inline-flex items-center justify-center gap-3 px-8 py-5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-black uppercase tracking-[0.2em] rounded-xl transition-all shadow-lg hover:shadow-indigo-500/30 hover:scale-105">
            Richiedi Accesso Ora <ArrowRight size={18} />
          </a>
        </section>
      </div>
      ...
      <footer className="w-full border-t border-indigo-100 dark:border-white/10 bg-white dark:bg-black py-8 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row items-center justify-between gap-6 text-xs font-bold text-slate-500 dark:text-slate-600 uppercase tracking-widest">
          <div className="flex items-center gap-3">
            <TerminalSquare size={16} /> CALI.AI INC.
          </div>
          <div className="flex gap-6">
            <a href="#" className="hover:text-indigo-600 transition-colors">Privacy</a>
            <a href="#" className="hover:text-indigo-600 transition-colors">Termini</a>
          </div>
          <div>EST. 2026</div>
        </div>
      </footer>
